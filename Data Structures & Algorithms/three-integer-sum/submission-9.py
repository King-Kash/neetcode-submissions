class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in range(len(nums)):
            if i > 0 and (nums[i] == nums[i-1]):# works since we sorted the list
                continue
            ...
            while left < right:
                if nums[left] + nums[right] == target:
                    ...
                    while nums[left] == nums[left-1] and left < right:
                        left += 1 #even though the indecies we consider will never duplicate the values might
                        # to avoid this we keep moving till we find a unique left val.
