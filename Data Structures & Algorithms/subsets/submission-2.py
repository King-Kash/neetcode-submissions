class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        ...
        recursive_res = self.subsets(nums[:-1])
        contains_last = []
        #print(recursive_res[0], last_element)
        for subset in recursive_res:
            new_subset = subset + [last_element]
            contains_last.append(new_subset)
        print(contains_last)
        return contains_last + recursive_res
